...
template<>
void PosePointIDPParametersBlock<7>::getPose(int idx, Quaterniond &q, Vector3d &trans)
{
    double* pose_ptr = values + idx * 7;
    q = Map<const Quaterniond>(pose_ptr);
    trans = Map<const Vector3d>(pose_ptr + 4);
...
template<>
void PosePointIDPParametersBlock<7>::setPose(int idx, const Quaterniond &q, const Vector3d &trans)
{
    double* pose_ptr = values + idx * 7;
    Eigen::Map<Vector7d> pose(pose_ptr);
    pose.head<4>() = Eigen::Vector4d(q.coeffs());
...
template<>
void PosePointIDPParametersBlock<6>::getPose(int idx, Quaterniond &q, Vector3d &trans)
{
    double* pose_ptr = values + idx * 6;
    q = toQuaterniond(Vector3d(pose_ptr));
    trans = Map<const Vector3d>(pose_ptr + 3);
...
template<>
void PosePointIDPParametersBlock<6>::setPose(int idx, const Quaterniond &q, const Vector3d &trans)
{
    double* pose_ptr = values + idx * 6;
    Eigen::Map<Vector6d> pose(pose_ptr);
    pose.head<3>() = toAngleAxis(q);
