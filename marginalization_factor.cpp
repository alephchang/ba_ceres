...
double MarginalizationInfo::eps = 1e-8;
...
MarginalizationInfo::~MarginalizationInfo()
{
    ...
    for (std::map<long, double *>::iterator it = parameter_block_data.begin(); it != parameter_block_data.end(); ++it)
        delete it->second;
...
std::vector<double *> MarginalizationInfo::getParameterBlocks()
{
    ...
    sum_block_size = std::accumulate(keep_block_size.begin(), keep_block_size.end(), 0);
...
{
    int cnt = 0;
    for (std::vector<int>::iterator it=marginalization_info->keep_block_size.begin(); it != marginalization_info->keep_block_size.end(); ++it)
    {
