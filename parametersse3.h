...
public:
    ...
    Vector3d cam_map_inv(const Vector2d& p, double rho){
        return Vector3d((p[0]-cx)/(f*rho), (p[1]-cy)/(f*rho), 1.0/rho);
    }
};
...
template<int PoseBlockSize>
class PosePointIDPParametersBlock //point is 1 dim
{
public:
    PosePointIDPParametersBlock(){}
    void create(int pose_num, int point_num)
    {
        poseNum = pose_num;
        pointNum = point_num;
        values = new double[pose_num * PoseBlockSize + point_num ];
    }
    PosePointIDPParametersBlock(int pose_num, int point_num): poseNum(pose_num), pointNum(point_num)
    {
        values = new double[pose_num * PoseBlockSize + point_num ];
    }
    ~PosePointIDPParametersBlock() { delete[] values; }
    ...
    double* point(int idx) { return values + poseNum * PoseBlockSize + idx ; }
